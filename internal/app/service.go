...
import (
	...
	"github.com/sudabon/dotenv_cryption/internal/format"
...
func NewService(secretProvider provider.SecretProvider) *Service {
	return &Service{secretProvider: secretProvider}
}
...
func (s *Service) EncryptFile(inputPath string, cfg config.Config) (string, error) {
	...
	masterKey, err := s.secretProvider.GetMasterKey()
	if err != nil {
		return "", err
	}
